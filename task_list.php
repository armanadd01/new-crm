			...
			<table class="table tabe-hover table-condensed" id="list">
				...
				<tbody>
					...
					while($row= $qry->fetch_assoc()):
                		...
                		$pdesc = strtr(html_entity_decode($row['pdesc']),$trans);
                		$pquan = strtr(html_entity_decode($row['pquan']),$trans);
						$pUrog = $tprog > 0 ? ($cprog/$tprog) * 100 : 0;
		                $pTrog = $prog > 0 ?  number_format($prog,2) : $prog;
					...
					<tr>
						...
						<td>
							...
							<p class="truncate"><b>Folder: </b><?php echo strip_tags($pdesc) ?></p>
						</td>
						<td>
							<span><b><?php echo ucwords($row['pclient']) ?></b></span>
						</td>
						<td>
							<span><b><?php echo ucwords($row['pquan']) ?></b></span>
						</td>
						<td>
							<p><b><?php echo ucwords($row['task']) ?></b></p>
							<p class="truncate"><b>File Type: </b><?php echo strip_tags($desc) ?></p>
						</td>
						<td><b><?php echo date("M d, Y",strtotime($row['start_date'])) ?></b></td>
						<td><b><?php echo date("M d, Y",strtotime($row['end_date'])) ?></b></td>
						<td><b><?php  echo number_format($dur).' Hr/s.' ?> </b></td>
						<td>
                        	<?php 
                        	if($row['status'] == 1){
