...
Class Action {
	...
	function save_employee(){
		...
		// echo $data;exit;
		if(empty($id)){
			$save = $this->db->query("INSERT INTO shift_list set $data");
		}else{
			$save = $this->db->query("UPDATE shift_list set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_employee(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM shift_list where id = $id");
		if($delete){
			return 1;
		}
	}
	...
	function save_progress(){
		...
		$dur = abs(strtotime("2021-01-01 ".$end_time)) - abs(strtotime("2021-01-01 ".$start_time));
		$dur = $dur / (60 * 60);
